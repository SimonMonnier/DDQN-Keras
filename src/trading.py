...
class TradingEnv(gym.Env):
    ...
    def __init__(self, dataset_path, spread, period, sold, min_sold, nlot, episode_lenght=1):
        ...
        self.state = np.append(np.append(np.append(self.episode_data[self.nstep:period], 0),0),0)
        self.total_trade = 0
        self.observation_space = spaces.Box(low=-300, high=300, shape=self.state.shape, dtype=np.float32)
